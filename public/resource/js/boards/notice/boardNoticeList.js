$(function() {
    ...
    var $buttonGroupDataTableNotice = $("#buttonGroupDataTableNotice");
    ...
    // 제목 또는 내용 검색
    $dropdownSearchOptionBoard.find('[name=btnSearchShort]').on("click", function(e) {
        searchShort();
    });
    ...
    // 일괄 삭제
    $buttonGroupDataTableNotice.find("[name=btnDelete]").on("click", function(e) {
        var selectedIds = $dataTableNotice.getSelectedIds('noticeId');
        ...
        var requestBody = {};
        if($dataTableNotice.isAllSelected()) {
     ...
     * 테이블 표시
     **************************************************************************/
    var displayBoardNoticePopup = ($("#displayBoardNoticePopup").val() == "true");

    var $dataTableNotice = $("#dataTableNotice").dataTableController({
        url : "/api/1/boards/notice",
        searchOption : searchOption,
        iDisplayLength : 20,
        buttonGroupId: "buttonGroupDataTableNotice",
        order : [ [ 1, 'desc' ] ],
        columnDefs: [ {
            targets: 0,
            orderable: false,
            width: "1%",
            className: "select-checkbox" + (sessionUserController.isAdmin()? "": " hide"),
            defaultContent: "",
        }, {
            targets: 1,
            data: "noticeId",
            ...
            render: function(data, type, row, meta) {
                ...
                var className = null;
                if (data == "N") {
                    className = "label-board-notice-status-notice";
                } else if (data == "F") {
                    className = "label-board-notice-status-faq";
                } else if (data == "E") {
                    className = "label-board-notice-status-etc";
                }
                return "<div class='label-board " + className + "'>" + messageController.get("item.board.notice.status." + data) + "</div>";
            }
        }, {
            targets: 3,
            data: "popupYn",
            ...
            className: "dt-head-center",
            visible: false,
            colvis: displayBoardNoticePopup,
            render: function(data, type, row, meta) {
                if (data != "Y") {
                    return "-";
                }
                return "<div class='label-board label-board-notice-popup'>" + messageController.get("label.popup.notice") + "</div>";
            }
        }, {
            targets: 4,
            data: "subject",
